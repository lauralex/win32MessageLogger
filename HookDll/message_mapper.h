...
private:
	std::unordered_map<DWORD, std::string> m_map_;
};

inline message_mapper::message_mapper() : m_map_({
...
inline std::string message_mapper::operator()(DWORD msg, LPARAM lp, WPARAM wp)
{
	constexpr auto first_col_width = 25;
	const auto i = m_map_.find(msg);

	std::stringstream oss;
	if (i != m_map_.end())
	{
		oss << std::left << std::setw(first_col_width) << i->second << std::right;
	}
	else
	{
		// std::stringstream padss;
